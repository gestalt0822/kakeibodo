class AnalysesController < ApplicationController
 def index
   ...
   @categories_then = current_user.bookings.where.not(date: Time.now.beginning_of_month..Time.now.end_of_month).select(:category_id).distinct#該当ユーザの家計簿のcategory_idを重複なしで配列に
