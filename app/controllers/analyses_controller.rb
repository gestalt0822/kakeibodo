class AnalysesController < ApplicationController
 def index
   @categories_now = current_user.bookings.where(date: Time.now.beginning_of_month..Time.now.end_of_month).select(:category_id).distinct#該当ユーザの家計簿のcategory_idを重複なしで配列に
   ...
   @categories_then = current_user.bookings.where(date: Time.now.prev_month.beginning_of_month..Time.now.prev_month.end_of_month).select(:category_id).distinct#該当ユーザの家計簿のcategory_idを重複なしで配列に
   ...
   while num < @categories_then.count do
     ...
     @each_amount << (current_user.bookings.where(category_id:@categories_then[num].category_id).sum(:amount).to_f/current_user.bookings.where(date: Time.now.prev_month.beginning_of_month..Time.now.prev_month.end_of_month).sum(:amount).to_f*100).round(2)
